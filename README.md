...
This project implements a **Time-Series Forecasting** approach to predict daily `StockPrice` using a **Linear Regression** model. The core assumption is that the stock price is primarily driven by the "Data" variable, specifically its previous day's value and immediate rate of change.

We assume a **non-stationary** relationship where the absolute level of `Data` shifts over time. To handle this, the model predicts the **Day-to-Day Price Change** (Price[t] - Price[t-1]) rather than the absolute price. The final forecasted price is reconstructed by adding the predicted change to the previous known price.
...
A **Linear Regression** model was selected for its interpretability and ability to capture the inverse correlation between the independent variable and the stock price.
...
2.  **Negative Correlation**: Both the absolute level (`Coef: -0.35`) and the rate of change (`Coef: -0.18`) have a negative impact on the stock price. This suggests that as the "Data" variable rises, the Stock Price falls.
...
# 2. Train the Model (Generates evaluation.json and test_results.csv)
python backend/train_model.py

# 3. Generate EDA Plots (Saved to backend/plots/)
python backend/eda.py
```

---
**Note**: Visualizations and detailed insights generated by the EDA script are stored in the `backend/plots/` directory.
